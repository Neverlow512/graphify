...
_smali_CLASS_RE: re.Pattern[str] = re.compile(
    r"^\.class\s+(?:(?:public|private|protected|abstract|final|interface|"
    r"enum|annotation|synthetic|bridge)\s+)*"
    r"(L[^;]+;)"
)
_smali_SUPER_RE: re.Pattern[str] = re.compile(r"^\.super\s+(L[^;]+;)")
_smali_IMPLEMENTS_RE: re.Pattern[str] = re.compile(r"^\.implements\s+(L[^;]+;)")
_smali_FIELD_RE: re.Pattern[str] = re.compile(
    r"^\.field\s+(?:(?:public|private|protected|static|final|volatile|transient|synthetic|enum)\s+)*([\w$<>\-]+):(.*)"
)
_smali_METHOD_START_RE: re.Pattern[str] = re.compile(
...
_smali_METHOD_END_RE: re.Pattern[str] = re.compile(r"^\.end\s+method")

_smali_INVOKE_RE: re.Pattern[str] = re.compile(
    r"^invoke-(?:virtual|direct|static|interface|super)"
    r"(?:/range)?\s+\{[^}]*\},\s*(\[+[BCDFIJSZ]|\[*L[^;]+;)->([\w$<>\-]+)\(([^)]*)\)(.*)"
)
_smali_INVOKE_POLY_RE: re.Pattern[str] = re.compile(
    r"^invoke-polymorphic(?:/range)?\s+\{[^}]*\},\s*(\[+[BCDFIJSZ]|\[*L[^;]+;)->([\w$<>\-]+)\(([^)]*)\)(.*?),\s*\("
)
_smali_INVOKE_CUSTOM_RE: re.Pattern[str] = re.compile(r"^invoke-custom\s+\{[^}]*\},\s*(\S+),\s*\(")

_smali_IGET_RE: re.Pattern[str] = re.compile(
    r"^iget(?:-wide|-object|-boolean|-byte|-char|-short)?\s+\S+,\s*\S+,\s*(\[*L[^;]+;)->([\w$<>\-]+):(.*)"
)
_smali_IPUT_RE: re.Pattern[str] = re.compile(
    r"^iput(?:-wide|-object|-boolean|-byte|-char|-short)?\s+\S+,\s*\S+,\s*(\[*L[^;]+;)->([\w$<>\-]+):(.*)"
)
_smali_SGET_RE: re.Pattern[str] = re.compile(
    r"^sget(?:-wide|-object|-boolean|-byte|-char|-short)?\s+\S+,\s*(\[*L[^;]+;)->([\w$<>\-]+):(.*)"
)
_smali_SPUT_RE: re.Pattern[str] = re.compile(
...
def _smali_outer_descriptor(descriptor: str) -> str | None:
...
def extract_smali(path: Path) -> dict:
    ...
    def _add_edge(
        ...
        # offsets are distinct runtime operations.
    ...
    for lineno, raw_line in enumerate(lines, start=1):
        ...
        if in_method:
            if _smali_METHOD_END_RE.match(line):
        ...
        m = _smali_IMPLEMENTS_RE.match(line)
        if m and current_class_nid:
            descriptor = m.group(1)
            label = _smali_descriptor_to_label(descriptor)
            tgt_nid = _make_id(label)
        ...
        m = _smali_METHOD_START_RE.match(line)
        if m and current_class_nid and current_class_descriptor:
    ...
    for method_nid, class_nid, body_lines in method_bodies:
        for lineno, line in body_lines:
            _smali_process_invoke(line, lineno, method_nid, _add_edge, _stub_node)
            _smali_process_field_access(line, lineno, method_nid, _add_edge, _stub_node)

    return {"nodes": nodes, "edges": edges, "input_tokens": 0, "output_tokens": 0}


def _smali_process_invoke(
    line: str,
    lineno: int,
    method_nid: str,
    add_edge: _smali_EdgeFn,
    stub_node: _smali_StubFn,
) -> None:
    m = _smali_INVOKE_RE.match(line)
    if m:
    ...
    m = _smali_INVOKE_POLY_RE.match(line)
    if m:
        class_desc = m.group(1)
        mname = m.group(2)
        class_label = _smali_descriptor_to_label(class_desc)
    ...
    m = _smali_INVOKE_CUSTOM_RE.match(line)
    if m:
...
def _smali_process_field_access(
    line: str,
    lineno: int,
    method_nid: str,
    add_edge: _smali_EdgeFn,
